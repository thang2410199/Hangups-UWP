...
namespace HangupsCore.Services
{
    public class Channel
    {
        ...
        public Channel(CookieContainer container)
        {
            _client = new HttpClient(new SigningMessageHandler(true, container));
        ...
        private void LongPollRequest()
        {
            ...
            HttpResponseMessage message = _client.Execute(HangoutsEndpoints.CHANNEL_URL + "channel/bind", headerData);
            message.EnsureSuccessStatusCode();
            dataReceived(message.Content.ReadAsStringAsync().Result);

            #region if streaming 
            //StringBuilder query = new StringBuilder(HangoutsEndpoints.CHANNEL_URL + "channel/bind?");
        ...
        void initialize()
        {
            ...
            HttpResponseMessage response = _client.Execute(HangoutsEndpoints.CHANNEL_URL + "gsid", null, null);
            ...
            response = _client.Execute(HangoutsEndpoints.CHANNEL_URL + "channel/cbp", headerData, null);
            response.EnsureSuccessStatusCode();
            message = response.Content.ReadAsStringAsync().Result;
        ...
        string sendMapsRequest(Dictionary<string, string> map_list = null)
        {
            ...
            HttpResponseMessage response = _client.Execute(HangoutsEndpoints.CHANNEL_URL + "channel/bind", headerData, map_list);
